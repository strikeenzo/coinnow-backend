...
class SellerApiController extends Controller
{
    ...
    {
        $specials = Special::select('quantity', 'product_id')
            ->where('seller_id', $this->getUser->id)
            ->where('quantity', '>', 0)
            ->with(
                'product:id,image,points',
                'productDescription:id,name,product_id'
            )
            ->get();
    ...
    public function sendCoins(Request $request)
    {
        ...
        if ($receiverID == $this->getUser->id) {
            return [
                'status' => 0,
                'message' => 'Unable to send coins to yourself.',
            ];
        }
        if ($amount < 1) {
            return [
            ...
            $history = Notification::select(
                ...
                ->with([
                    'product' => function ($query) {
                        $query
                            ->select('id', 'image')
                            ->with('productDescription:id,name,product_id');
                    },
                ])
                ->where('seller_id', $this->getUser->id)
    ...
    public function searchProducts(Request $request)
    {
        try {
            ...
            $records = Product::select(
                ...
                ->where('seller_id', $this->getUser->id)
                ->with(
                    'productDescription:name,id,product_id',
                    'special:product_id,price,start_date,end_date'
                )
                ->when(!empty($keyword), function ($q) use ($keyword) {
                    $q->whereHas('productDescription', function ($q) use (
                        $keyword
                    ) {
    ...
    public function trade(Request $request)
    {
        ...
        $product = Product::where('seller_id', $this->getUser->id)
            ->where('origin_id', $request->get('product_id'))
            ->get();
        // return $product;
        if (count($product) == 0) {
            return ['status' => 3];
        ...
        try {
            $product = Product::where('seller_id', $this->getUser->id)
                ->where('origin_id', $request->get('product_id'))
                ->decrement('quantity', $request->get('quantity_trade'));
            //error_log(count($product));
            Seller::find($this->getUser->id)->increment(
                'balance',
                $request->get('coin_quantity')
            );
            Trade::find($request->get('id'))->decrement('quantity', 1);

            $amount = Seller::where('id', $this->getUser->id)
                ->select('balance')
                ->get();
            ...
            $notification = new Notification([
                ...
                'type' => 'trade',
                'amount' => $request->get('coin_quantity'),
                'seller_id' => $this->getUser->id,
                'balance' => $amount,
            ]);
            $notification->save();

            $trade_count = Trade::find($request->get('id'))
                ->select('quantity')
                ->get();
    ...
    public function updateProfile(Request $request)
    {
        try {
            ...
            $update = Seller::where('id', $this->getUser->id)->update(
                $request->all()
            );
            $getNew = Seller::select(
                'firstname',
                'lastname',
                'email',
                'store_name',
                'telephone'
            )->findOrFail($this->getUser->id);
            return [
                'status' => 1,
