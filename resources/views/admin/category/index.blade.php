                        ...
                        <table class="table align-items-center table-flush">
                            ...
                            @forelse($records as $key => $value)
                                ...
                                <tr>
                                  <td class="budget">
                                    @if($value->image)
                                      <img src="{{asset('/public/uploads/category').'/'.$value->image}}"  alt="{{$value->categoryDescription->name}}"  class="img-thumbnail img-fluid" style=" width: 60px;height: 60px;">
                                    @else
                                      <img src="{{asset('/assets/img/default.png')}}"  alt="{{$value->name}}"  class="img-thumbnail img-fluid" style=" width: 60px;height: 60px;">
                                    @endif
                                  </td>
                                    <td class="budget">{{ $value->categoryDescription->name }}</td>
                                    <td class="budget">{{ $value->parent_id == 0 ? 'Yes' : 'No' }}</td>
                                    <td class="budget">{{ $value->categoryDescription->meta_title }}</td>
                                    ...
                                    <td class="">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                                <a class="dropdown-item" href="{{ route('category.edit',['id' => $value->category_id]) }}">Edit</a>
                                                <a class="dropdown-item deleteData" type="button"  href="javascript:void(0)" data-url="{{ route('category.delete',['id' => $value->category_id]) }}">Delete</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
