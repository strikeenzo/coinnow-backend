<div class="tab-pane active" id="tab-general">
    ...
    <div class="col-md-12 form-group{{ $errors->has('image_profit') ? ' has-danger' : '' }}">
        <label class="form-control-label" for="input-name">{{ __('Image Profit') }}*</label>
        <input type="number" name="image_profit" id="input-name"
            class="form-control form-control-alternative{{ $errors->has('image_profit') ? ' is-invalid' : '' }}"
            value="{{ old('image_profit', 0) }}" autofocus required>

        @if ($errors->has('image_profit'))
            <span class="invalid-feedback" role="alert">
                <strong>{{ $errors->first('image_profit') }}</strong>
            </span>
        @endif
    </div>
