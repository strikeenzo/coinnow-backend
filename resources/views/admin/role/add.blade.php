                ...
                <div class="card bg-secondary shadow">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('role.store') }}"  autocomplete="off">
                            ...
                            <h6 class="heading-small text-muted mb-4">{{ __('Add Role ') }}</h6>

                            <div class="pl-lg-4 row">
                                <div class="col-md-4 form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-name">{{ __('Name') }}</label>
                                    <input type="text" name="name" id="input-name" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}"  value="{{ old('name', '') }}" autofocus>
...
{{--                                </div>--}}
                                    ...
                                    @foreach($val as $key1 => $val1)
                                        <div class="col-md-2 form-group">
                                            <label class="checkbox-inline">
                                                <input type="checkbox" name="permissions[]"  value="{{ "$key.$key1" }}"> {{ $val1 }}
                                            </label>
                                        </div>
                                    @endforeach
                            ...
                            <div class="pl-lg-4 row">
                                ...
                                <div class="text-center">
                                    ...
                                    <a href="{{ route('role') }}" type="button" class="btn btn-danger mt-4">{{ __('Cancel') }}</a>
                                </div>
                            </div>
                        </form>
                    </div>
