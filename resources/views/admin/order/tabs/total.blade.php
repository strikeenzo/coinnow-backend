            ...
            <div class="form-group">
                ...
                <select class="form-control" id="shipping_method" name="shipping_method">
                    ...
                    @foreach (config('constant.shipping_method') as $key => $value)
                        <option
                            {{ isset($order->shipping_method) && $order->shipping_method == $key ? 'selected' : '' }}
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="payment_method" name="payment_method">
                    ...
                    @foreach (config('constant.payment_method') as $key => $value)
                        <option value={{ $key }}
                            {{ isset($order->payment_method) && $order->payment_method == $key ? 'selected' : '' }}>
                            {{ $value }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="order_status_id" name="order_status_id">
                    @foreach ($data['order_status'] as $key => $value)
                        <option value={{ $key }}
                            {{ isset($order->order_status_id) && $data['order_status'][$order->order_status_id] == $key ? 'selected' : '' }}>
                            {{ $value }}</option>
                    @endforeach
                </select>
            </div>
