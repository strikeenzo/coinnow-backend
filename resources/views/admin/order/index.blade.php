            ...
            <div class="col">
                <div class="card">
                    <div class="card-header border-0">
                        ...
                        <button type="button" onclick="window.location.href = '{{ route('order') . '?filterStatus=all' }}'"
                            class="btn @if (!Request::get('filterStatus') || Request::get('filterStatus') == 'all') btn-outline-info @else btn-info @endif "><i
                                class="fa fa-list" aria-hidden="true"></i> All</button>
                        @foreach ($orderStatus as $key => $status)
                            <button type="button"
                                onclick="window.location.href = '{{ route('order') . '?filterStatus=' . $status }}'"
                                class="btn @if (!Request::get('filterStatus') || Request::get('filterStatus') == $status) btn-primary @else btn-info @endif "><i
                                    class="fa fa-list" aria-hidden="true"></i> {{ $status }}</button>
                        @endforeach
                            ...
                            <tbody class="list">
                                ...
                                @forelse($records as $key => $value)
                                    <tr>
                                        ...
                                        <td class="budget">{{ $value->orderStatus ? $value->orderStatus->name : null }}
                                        </td>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                                    <a class="dropdown-item"
                                                        href="{{ route('order.view', ['id' => $value->id]) }}">View</a>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
