                      ...
                      <div class="card-header border-0">
                          ...
                          <button type="button" onclick="window.location.href = '{{route('order').'?filterStatus=all'}}'" class="btn @if(!Request::get('filterStatus') || Request::get('filterStatus') == 'all') btn-outline-info @else btn-info @endif "><i class="fa fa-list" aria-hidden="true"></i> All</button>
                            @foreach($orderStatus as $key => $status)
                              <button type="button" onclick="window.location.href = '{{route('order').'?filterStatus='.$status}}'" class="btn @if(!Request::get('filterStatus') || Request::get('filterStatus') == $status) btn-primary @else btn-info @endif "><i class="fa fa-list" aria-hidden="true"></i> {{$status}}</button>
                            @endforeach
