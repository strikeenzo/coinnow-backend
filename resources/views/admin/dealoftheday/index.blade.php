...
@section('content')
    ...
    <div class="container-fluid">
        <div class="row  ">
            <div class="col">
                <div class="card">
                    <form method="post" id="data-form" action="{{ route('trending_dod.store') }}"
                        enctype="multipart/form-data" autocomplete="off">
                        @csrf
                        @method('post')
                        <div class="col-md-10  mt-3 ml-8 form-group{{ $errors->has('product_id') ? ' has-danger' : '' }}">
                            <label class="form-control-label"
                                for="parent_id">{{ __('Add Product To Deals Of The Day') }}</label>
                            <select class="form-control selectpicker " multiple data-live-search="true" name="product_id[]">
                                @foreach ($data['pluckProducts'] as $key => $value)
                                    <option value={{ $key }}
                                        {{ in_array($key, $data['pluckDODProducts']) ? 'selected' : '' }}>{{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="text-center">
                            <button type="submit" class="btn btn-success btn-lg mb-3"><i class="fa fa-plus"></i>
                                {{ __('Add Selected Products To Deals Of The Day') }}</button>
                        </div>
                    </form>
                </div>
            </div>
                                    ...
                                    <tr>
                                        <td>
                                            @if ($value->productDetails && $value->productDetails->image)
                                                <img src="{{ asset('/public/uploads/product') . '/' . $value->productDetails->image }}"
                                                    alt="{{ $value->productDescription->name }}"
                                        ...
                                        <td class="budget">
                                            {{ $value->productDescription ? $value->productDescription->name : '' }}</td>
                                        <td class="budget">${{ $value->productDetails->price }}</td>
                                        <td class="budget">
                                            ${{ $value->productSpecial ? $value->productSpecial->price : 0 }}</td>
                                    </tr>
                                @empty
                                    <tr>
