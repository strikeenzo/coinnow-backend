                          ...
                          <form action="{{ route('product') }}">
                              <div class="row">
                                  <div class="col-md-3 form-group">
                                      <input type="text" name="name" id="name" value="{{ request()->name }}" class="form-control form-control-alternative" placeholder="Search by name" autofocus>
                                  </div>
                                  <div class="col-md-3 form-group">
                                      <input type="text" name="model" id="model" value="{{ request()->model }}" class="form-control form-control-alternative" placeholder="Search by model" >
                                  </div>
                                  <div class="col-md-2 form-group">
                                      <input type="number" name="quantity" id="quantity" value="{{ request()->quantity }}" class="form-control form-control-alternative" placeholder="Search by quantity" >
                                  </div>

                                  <div class="col-md-2 form-group">
                                    <select class="form-control" name="status">
                                      <option value="1" @if($status == 1 ) selected="true"   @endif >Active</option>
                                      <option value="0" @if($status == 0)  selected="true" @endif>DeActive</option>
                                    </select>
                                  </div>
                                  <div class="col-md-2 form-group">
                                      <button type="submit" class="btn btn-success"><i class="fas fa-search"></i></button>
                                      <a href="{{ route('product') }}" class="btn btn-info"><i class="fas fa-sync-alt"></i></a>
                                  </div>
                              </div>
                          </form>
                      </div>
                ...
                <div class="card">
                    ...
                    <div class="table-responsive">
                        <table class="table align-items-center table-flush">
                            ...
                            <tr>
                                <th style="font-size: 10px" scope="col" class="sort" data-sort="name" style="font-size: 10px">Product</th>
                                <th style="font-size: 10px" scope="col" class="sort" data-sort="name">Name</th>
                                <th style="font-size: 10px" scope="col" class="sort" data-sort="status">Category</th>
                            ...
                            @forelse($records as $key => $value)
                                <tr>
                                    ...
                                    <td class="budget "  > <span class="@if($value->clan_members < 5) badge badge-danger btn-small @else badge badge-success btn-small @endif" style="font-size:12px;">{{ $value->clan_members }}</span></td>
